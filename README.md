...
![](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=20&pause=1000&color=3C6E52&center=true&vCenter=true&width=500&lines=180+questions.+12+chapters.;SELECT+%C2%B7+WHERE+%C2%B7+JOIN+%C2%B7+GROUP+BY...;Write+every+query+yourself.)
...
180 questions across 12 chapters. Each bar below is a real SVG with a moving shine animation, generated by `assets/gen_svgs.py`. To update your count after a study session: open `gen_svgs.py`, change a chapter's `done` value, rerun `python3 gen_svgs.py`, and commit the regenerated files in `assets/` — the bars will fill to the new amount and keep animating.

---

## Chapter 1 — SELECT

![](assets/ch01_select.svg)
...
![](assets/ch04_between_in_not.svg)
...
- Update a chapter's progress-bar number only after solving that many questions.
- Revisit questions that need extra practice.

## Topics Covered

SELECT · WHERE · AND / OR · BETWEEN / IN / NOT · ORDER BY · LIMIT · COUNT / SUM / AVG / MAX / MIN · GROUP BY · HAVING · JOIN · LIKE · DISTINCT
